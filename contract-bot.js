...
client.on('interactionCreate', async interaction => {
    // Gestion du bouton de création dans le salon de proposition
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'modal_create') {
        ...
        const infoEmbed = new EmbedBuilder()
            ...
            .setColor('#f1c40f')
            .addFields({ name: '💰 Budget', value: `${budget}€`, inline: true }, { name: '⏳ Délai', value: deadline, inline: true }, { name: '📝 Description', value: desc });

        const safetyEmbed = new EmbedBuilder()
            .setTitle("🛡️ PROTECTION & SÉCURITÉ")
            .setColor('#ff0000')
            .setDescription(`Jamais un secrétaire ne vous demandera de payer directement. Tout passe par le PayPal officiel HEO.\n\nEn cas de doute : @🚨 • Urgence`);
    ...
    // Gestion du Modal de choix du Développeur
    if (interaction.isModalSubmit() && interaction.customId === 'modal_dev') {
        ...
        const devId = devTag.replace(/[<@!>]/g, ''); // Nettoyer l'ID si c'est une mention
        ...
        const ticketDev = await interaction.guild.channels.create({
            ...
            permissionOverwrites: [
                { id: interaction.guild.id, deny: [PermissionFlagsBits.ViewChannel] },
                { id: devId, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] }, // Ajout du dev
                { id: CONFIG.ROLE_SECRETAIRE, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] },
                { id: CONFIG.ROLE_OWNER, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] },
            ]
        });
        ...
        await interaction.editReply({ content: "Développeur assigné et ajouté au ticket." });
    }

    // Boutons de navigation
    if (interaction.isButton() && ['next_step', 'back_step', 'cancel_contract'].includes(interaction.customId)) {
        const category = interaction.channel.parent;
        const state = contractStates.get(category.id);
        if (!state) return;
        ...
        if (interaction.customId === 'next_step') {
            if (state.step === 0) {
                ...
                modalDev.addComponents(
                    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('dev_tag').setLabel("Tag du Dev (@exemple)").setStyle(TextInputStyle.Short).setRequired(true)),
            ...
            state.step++;
            await updateWorkflow(category, state);
            await interaction.reply({ content: "Étape suivante...", ephemeral: true });
        }
        ...
        if (interaction.customId === 'back_step') {
            ...
            await updateWorkflow(category, state);
            await interaction.reply({ content: "Retour en arrière...", ephemeral: true });
        }
